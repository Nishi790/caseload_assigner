...
@export var site_error_review: ErrorReviewer
...
func _ready() -> void:
	...
	site_error_review.tab_id = get_tab_idx_from_control(site_error_review)
	...
	tab_changed.connect(site_error_review._on_return_to_tab)
